...
router.post("/", requireAuth("admin"), async (req,res)=>{
  const { name, niche, package:pkg, businessId, username, passcode, modules } = req.body || {};
  if(!name) return res.status(400).json({error:"name is required"});
  const id = String(businessId || nanoid(10)).trim();
  const loginUsername = String(username || id).trim();
  const setupPasscode = String(passcode || Math.random().toString(36).slice(2,8).toUpperCase()).trim();
  if(!id) return res.status(400).json({error:"Business ID is required"});
  if(!loginUsername) return res.status(400).json({error:"Username is required"});
  if(!setupPasscode) return res.status(400).json({error:"Client passcode is required"});
  const existing = read().businesses.find(b => b.id === id || b.username === loginUsername);
  if(existing) return res.status(409).json({error:"Business ID or username already exists. Choose another one."});
  const selectedModules = modules && typeof modules === "object" ? Object.fromEntries(Object.entries(DEFAULT_MODULES).map(([key,defaultValue]) => [key, modules[key] === undefined ? defaultValue : !!modules[key]])) : {...DEFAULT_MODULES};
  const biz={id,username:loginUsername,name,niche:niche&&listNiches().some(n=>n.id===niche)?niche:"generic",package:PACKAGES[pkg]?pkg:"starter",modules:selectedModules,passcodeHash:bcrypt.hashSync(setupPasscode,8),createdAt:new Date().toISOString()};
  await transact(d=>d.businesses.push(biz));
  res.status(201).json({business:publicBiz(biz),passcode:setupPasscode});
});
...
router.get("/:id/public-config", (req,res)=>{const biz=read().businesses.find(b=>b.id===req.params.id);if(!biz)return res.status(404).json({error:"Business not found"});res.json({id:biz.id,username:biz.username||biz.id,name:biz.name,niche:biz.niche,modulesEnabled:Object.fromEntries(Object.entries(biz.modules||{}).filter(([,enabled])=>enabled))});});
