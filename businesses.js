...
router.post("/", requireAuth("admin"), async (req,res)=>{
  const { name, niche, package:pkg, businessId, passcode } = req.body || {};
  ...
  const existing = read().businesses.find(b => b.id === id);
  if(existing) return res.status(409).json({error:"Business ID already exists. Choose another one."});
  const biz={id,name,niche:niche&&listNiches().some(n=>n.id===niche)?niche:"generic",package:PACKAGES[pkg]?pkg:"starter",modules:{...DEFAULT_MODULES},passcodeHash:bcrypt.hashSync(setupPasscode,8),createdAt:new Date().toISOString()};
