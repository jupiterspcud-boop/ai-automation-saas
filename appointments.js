...
// PUBLIC: book an appointment (called by the widget after the chatbot flow,
// or by any booking form on the client's site).
router.post("/businesses/:businessId/appointments", async (req, res) => {
  ...
  const appt = {
    ...
    time: time || "",
    notes: notes || "",
    status: "booked", // booked -> reminded -> completed -> cancelled
    createdAt: new Date().toISOString(),
  };

  await transact((d) => d.appointments.push(appt));
